...
def _contains_bc_indicator(answer:str):
...
def preprocess_numeric_player_ans(raw_player_ans:str,
    ...
    if answer_type==AnswerType.YEAR:
        is_bc = _contains_bc_indicator(ans_text)
...
def check_numeric_answer(player_answer_num: int | None,
    ...
    correct_answer = preprocess_numeric_player_ans(q.answer, q.answer_type)

    if correct_answer is None:
        # failsafe: Prevent an IndexError if gold_answer contains no digits
        result = BaseEvalResults(is_correct = False,
                               resolution_tier = 'numeric_exact_fail_invalid_gold')
        emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
        return result

    # 2. if the player answer did not have any numbers
    # TODO: Distinguish preprocessing failure modes (e.g. hedging,
    # parse failure, missing numeric entity) into separate preprocessing 
    # resolution states instead of collapsing to None. Use flag from preproessor.
    if player_answer_num is None:
        result = BaseEvalResults(is_correct = False,
                                 resolution_tier = 'numeric_exact_fail_invalid_or_no_num')
        emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
        return result

    # 3. check if the correct number is provided
    if player_answer_num == correct_answer:
        result = BaseEvalResults(is_correct = True,
                                 resolution_tier = 'numeric_exact_pass')
        emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
        return result

    # 4. catch all fail
    result = BaseEvalResults(is_correct = False,
                             resolution_tier = 'numeric_exact_fail')
    emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
    return result
...
def check_date_answer(player_entities: dict[str, int | None] | None, 
    ...
    if player_entities is None:
        result = BaseEvalResults(is_correct = False, 
                                 resolution_tier = 'date_exact_fail_invalid_player_ans')
        emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
        return result
    ...
    if player_entities == gold_entities:
        result = BaseEvalResults(is_correct = True,
                                 resolution_tier = 'date_exact_pass')
        emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
        return result

    # 4. catch all fail
    result = BaseEvalResults(is_correct = False,
                             resolution_tier = 'date_exact_fail')
    emit_eval_log(result, q.master_id, q.question_type,q.answer_type, logger)
    return result
