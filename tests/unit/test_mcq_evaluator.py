...
# actual sample from validated runtime df - complete set for reference (will delete after test suite is complete)
MCQ_SAMPLE = {
...
def mock_check_semantic_variations(player_tensor, gold_tensor, variation_tensor):
    return 0.92, "fiendfyre"   # correct score + matched variation
...
@patch("engine.evaluators.mcq_evaluator.check_semantic_variations", side_effect=mock_check_semantic_variations)
...
def test_mcq_evaluator_player_answer_is_bool():
    ...
    with pytest.raises(TypeError, match="player_answer must be a normalized string"):
        check_mcq_answer(player_answer, q)  # type:ignore - wrong type used for test
