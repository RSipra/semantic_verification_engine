...
def tag_questions_by_keyword_list(df: pd.DataFrame,
    ...
    # creates a list of keywords found otherwise returns an empty list 
    tag_results = df_copy[keyword_column].apply(
        lambda tokens: [word for word in trigger_keyword_list if word in tokens]
    )
...
def get_main_keyword(tag_list):
    ...
    if isinstance(tag_list, list):
        if len(tag_list) > 0:
            # Case 1: It's a list with keywords in it.
            return tag_list[0]
        else:
            # Case 2: It's an empty list.
            return 'Non-Factual'

    # Case 3: It wasn't a list at all (a safe fallback).
    return 'Other'
